...
namespace Stocktopus {
    internal class Tables {
        ...
        public static int GetValue(char piece, int i, GameState state) {
            if (state == GameState.Midgame) {
                switch (piece) {
                    case 'p': case 'P': return MidgameTables[i];
                    case 'n': case 'N': return MidgameTables[i + 64];
                    case 'b': case 'B': return MidgameTables[i + (2 * 64)];
                    case 'r': case 'R': return MidgameTables[i + (3 * 64)];
                    case 'q': case 'Q': return MidgameTables[i + (4 * 64)];
                    case 'k': case 'K': return MidgameTables[i + (5 * 64)];
                }
            } else {
                switch (piece) {
                    case 'p': case 'P': return EndgameTables[i];
                    case 'n': case 'N': return EndgameTables[i + 64];
                    case 'b': case 'B': return EndgameTables[i + (2 * 64)];
                    case 'r': case 'R': return EndgameTables[i + (3 * 64)];
                    case 'q': case 'Q': return EndgameTables[i + (4 * 64)];
                    case 'k': case 'K': return EndgameTables[i + (5 * 64)];
                }
            }
            return -1;
        }
