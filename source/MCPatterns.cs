...
namespace Stocktopus {
    internal static class MCPatterns {
        ...
        public static List<Move> GetBishopMoves(in Board<Piece?> board, int i) {
            List<Move> moves = new List<Move>();

            int[] pattern = new int[] { 7, 9 };
            ...
            foreach (int p in pattern) {
                // Up
                lFile = file;
                for (int j = i - p; j > i - 100; j -= p) {
                    nFile = Utils.IndexToXY(j).Item1;
                    if (j >= 0 && j < 64 && Utils.Max(lFile, nFile) - Utils.Min(lFile, nFile) == 1 && board[j] == null) {
                        moves.Add(new Move(i, j, false));
                        lFile = nFile;
                    } else if (j >= 0 && j < 64 && Utils.Max(lFile, nFile) - Utils.Min(lFile, nFile) == 1 && board[j] != null && board[j].color != board[i].color) {
            ...
            //    Console.WriteLine($"{m.start} {m.end}");

            return moves;
        }

        public static List<Move> GetRookMoves(in Board<Piece?> board, int i) {
            List<Move> moves = new List<Move>();

            int[] pattern = new int[] { 1, 8, -1, -8 };
